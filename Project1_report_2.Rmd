...
plot.weekdays <- function(df, region_name){
  x <- df %>% filter(!is.na(speed) & region == region_name)
  x <- x %>% mutate(from = paste(from,to,sep = "-")) %>% select(-to)
  colnames(x)[3] = "line"
  d <- x$date %>% date %>% weekdays %>% as.factor
  g <- x %>% ggplot(aes(x=d, y=speed, col = d)) + geom_boxplot() + geom_jitter() + ggtitle(paste(region_name,"region"))
  g
}
...
ptrn.user.reports <- c(".*(\\d)+.*دقا*[يئ]ق", 
                       ".*(\\d)+m.*n.*t?s?")
ptrn.list.2 <- list(
                  "user_reports" = ptrn.user.reports
        				  )
...
user.reports.df <- scratch.df %>% filter(category == "user_reports")
scratch.df <- scratch.df %>% filter(category != "user_reports")
...
```{r}
plot.weekdays(gps.reports.df, "Da2ery")
```

```{r}
plot.weekdays(gps.reports.df, "Me7war")
```

```{r}
plot.weekdays(gps.reports.df, "Sa7rawy")
```

```{r}
plot.weekdays(gps.reports.df, "Autostrad")
```

```{r}
plot.weekdays(gps.reports.df, "Kobry 6 October")
```

```{r}
plot.weekdays("Kobry Kasr ElNile")
```
