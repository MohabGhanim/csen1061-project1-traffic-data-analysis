...
```{r}
n <- in.city.df %>% group_by(region) %>% summarize(n=n()) %>% arrange(desc(n))
print(x)
# So the most three most big numbers are Da2ery, kobry 6 October
```

```{r}
day.heat.map <- function(df, region_name){
  x <- df %>% filter(region == region_name) %>% 
    mutate(date = hour(date)) %>% 
    mutate(line = paste(from,to,sep="-")) %>%
    select(date,line,category)
    print(nrow(x))
 cats <- c("7alawa","lazeez","mashy","z7ma","mfesh_amal","7adsa","3otl")
 cats.as.nums <- x$category %>% match(cats)
 
 x <- x %>% mutate(category = cats.as.nums) %>%
   group_by(date,line) %>% 
   summarize(avg.cat = mean(category)) %>% 
   spread(line, avg.cat)
 
 x <- x[,-1]
 x <- x %>% as.matrix
 levelplot(x, col.regions=colorpanel(16,"moccasin","orangered3"),
           scales = list(x = list(at=c(0:24),
                                  labels=c(0:24)
                                  )
                         )
           )
...
day.heat.map(in.city.df, "Da2ery")
day.heat.map(in.city.df, "Kobry 6 October")
day.heat.map(in.city.df, "Sala7 Salem")
day.heat.map(in.city.df, "Me7war")
