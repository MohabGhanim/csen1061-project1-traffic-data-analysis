...
```{r}
cats <- c("7alawa","lazeez","mashy","z7ma","mfesh_amal","7adsa","khatar", "3otl")
n <- in.city.df %>% select(date, region, category)
n$date <- hour(n$date)
n$category <- sapply(n$category, function(l){
  ifelse(length(which(l == cats))>0,which(l == cats), NA)})
