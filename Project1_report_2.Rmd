...
View.subset <- function(dataframe,n=10){
  ...
  View(dataframe[rndm,c("comment","category")],n)
}
...
df <- df %>% mutate(comment = as.character(comment))
...
colnames(scratch.df) <- c("date","region","from","to","road_id","is_high_way","reporter","tag","comment")
...
has.category <- with(scratch.df,sapply(comment, get.category, patterns = ptrn.vector, names = ptrn.index))
